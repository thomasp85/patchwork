...
patchwork's biggest selling point is that it expands on the use of `+` in ggplot2
...
operators shown above. This can lead to surprising behaviour:
...
This behaviour is necessary to allow patchworks to be build up gradually, but
...
There are currently no version of `/` and `|` that has this behaviour, so it is
necessary to specify `ncol = 1`/`nrow = 1` explicitly in `plot_layout()` to get
this behaviour with left-hand side nesting.
