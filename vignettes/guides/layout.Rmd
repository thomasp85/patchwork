...
`coord_sf()`. It is not possible to simulatneausly assign even dimensions and 
...
will behave just as `1null`, but if the row/column is occupied by a fixed aspect 
plot it will expand and contract in order to keep the aspect of the plot.
...
```{r}
p_fixed + p1 + p2 + p3 + plot_layout(widths = 1, heights = 1)
```
...
dependent on the specific use case. The default optimises the use of space.

There are some restrictions to the space optimisation. The fixed aspect plot ¨
